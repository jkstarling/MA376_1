...
plot.data %>% ggplot() + aes(x = Participant, y = Mean, group = HurrGender, color = HurrGender) +
...
interact.lm <- lm(Rating ~ HurrGender * Participant, data=hurrA)
summary(interact.lm)
anova(interact.lm)
```
...
plot.data %>% ggplot() + aes(x = HurricaneGender, y = Mean, group = HurricaneAge, color = HurricaneAge) +
              geom_line() + geom_point() + theme_classic()
```
...
gender.lm <- hurrB %>% lm(Rating ~ HurricaneGender + HurricaneAge, data = .)
...
hurrB %>% group_by(HurricaneGender, HurricaneAge) %>% summarise(n=n())
