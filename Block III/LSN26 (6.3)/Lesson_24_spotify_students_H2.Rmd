---
title: "Lesson 24 Spotify Cadets"
...
# class(music$like)
music %>%
  ggplot(aes(y=danceability))+geom_boxplot()+facet_wrap(~like)+labs(title="Boxplot of Like to Danceability")

```

```{r,fig.height=3, fig.width=5, fig.cap="Like vs. Key"}
music %>%
  ggplot(aes(y=key))+geom_bar()+facet_wrap(~like)+labs(title="Boxplot of Like to key")
...
Based on the box plot, the user tends to like songs that have a higher danceability. Based on the bar chart, the values for key were similar for users regardless of whether they liked or disliked a song.
...
$\log(\frac{\pi}{1-\pi}) = \beta_0 + \beta_1(\text{instrumentality}) + \beta_2(\text{key}) + \beta_3(\text{danceability}) + \beta_4(\text{mode}) + \beta_5(\text{tempo})$
...
model.lm <- glm(like~danceability+instrumentalness+key+mode+tempo, data= music, family="binomial")
summary(model.lm)
```
...
```{r}
# Interaction model
int.glm <- music %>% glm(like~instrumentalness + key + danceability + mode*tempo, data=., family='binomial')
summary(int.glm)
...
test_songs <- test_songs %>% mutate(mode = as.factor(mode))%>% mutate (key=as.factor (key))

# Predicted probability based on preferred model
library (broom)

like_probability <- model.lm %>%
  augment (type.predict= "response", newdata= test_songs)%>%
  rename (like_probability = .fitted)%>%
  select (title, like_probability)

like_probability

predict.glm(model.lm, test_songs, type = "response")
