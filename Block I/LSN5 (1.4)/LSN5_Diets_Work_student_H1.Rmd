...
```{r Explore the Data}
diet.means <- dietdat %>% group_by(diet) %>% summarise(n=n(), Mean = mean(wloss), SD=sd(wloss))
diet.means
...
# Create a violin plot
 dietdat %>% ggplot(aes(x=wloss,y=diet)) + geom_violin() + geom_boxplot(width=0.15) + geom_point()
...
SSE <- sum(  (diet.lm$fitted.values - dietdat$wloss)^2  ); SSE
...
data.frame(stat) %>% ggplot(aes(x=stat)) + geom_histogram() + 
