...
```{r}
afghan %>% ggplot(aes(x = test_score)) +
  geom_dotplot(dotsize = 0.7,binwidth = 1) +
  facet_wrap(~treatment) +
  labs(x = "Test Scores (Points)", title = "Dot plot of test scores (stratified by treatment)") +
  theme_classic()
```
\vspace{1in}
...
afghan %>% ggplot(aes(x = test_score)) +
  geom_histogram() +
  facet_wrap(~treatment) +
  labs(x = "Test Scores (Points)", title = "Histogram of test scores (stratified by treatment)") +
  theme_classic()
```
\vspace{1in}
...
```{r}
afghan %>% group_by(treatment) %>% summarise(Mean = mean(test_score), SD = sd(test_score))
```
\vspace{1in}
...
afghan %>% ggplot(aes(x = age)) +
  ...
  labs(x = "Age (years)", title = "Histogram of age by treatment type") +
  theme_classic()
```
\vspace{1in}
...
```{r}
afghan %>% ggplot(aes(x = as.factor(girl), y = test_score, fill=as.factor(treatment))) +
  geom_boxplot() +
  labs(x = "Sex ( 1 = female, 0 = male)", title = "Box plot of test scores by treatment type and sex") +
  theme_classic()
```
\vspace{1in}
