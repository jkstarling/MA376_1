...
cereal <- cereal %>% select(Rating.of.cereal, grams.of.sugars, calories.per.serving)
```
...
```{r}
sugar.lm = cereal %>% lm(Rating.of.cereal~grams.of.sugars, 
                         data = .)
summary(sugar.lm)
calories.lm<-cereal %>% lm(Rating.of.cereal~calories.per.serving, 
                           data = .)
summary(calories.lm)
```
...
both.lm <- cereal %>% lm(Rating.of.cereal~grams.of.sugars+
...
```{r}
cereal_std <- cereal %>%
  mutate_at(list(~scale(.)), 
            .var = vars(grams.of.sugars, calories.per.serving))

# plot regular and standardized data
cereal %>% ggplot(aes(x=grams.of.sugars, y=calories.per.serving)) +                     geom_point() + expand_limits(x=0,y=0)

cereal_std %>% ggplot(aes(x=grams.of.sugars, y=calories.per.serving)) +  geom_point()
