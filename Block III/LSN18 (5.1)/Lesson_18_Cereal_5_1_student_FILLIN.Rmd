...
```{r}
sugar.lm <- cereal %>% lm(FILL.IN.HERE)
summary(sugar.lm)

calories.lm <- cereal %>% lm(FILL.IN.HERE)
summary(calories.lm)
```
...
```{r}
both.lm <- cereal %>% lm(FILL.IN.HERE)
summary(both.lm)
```
...
p.reg <- cereal %>% ggplot(FILL.IN.HERE) + 
  geom_point() + 
  expand_limits(x = 0, y = 0)

p.std <- cereal_std %>% ggplot(FILL.IN.HERE) + 
  geom_point()
...
```{r}
std.lm <- cereal_std %>% lm(FILL.IN.HERE)
summary(std.lm)

#must calculate to fully interpret coefficients
cereal %>% select(FILL.IN.HERE) %>% 
...
```{r}
inter.lm <- cereal_std %>% lm(FILL.IN.HERE)
summary(inter.lm)
