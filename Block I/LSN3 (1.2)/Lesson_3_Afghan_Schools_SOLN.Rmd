...
```{r}
gp0 <- afghan %>% filter(treatment=="0") %>% select(test_score)
gp1 <- afghan %>% filter(treatment=="1") %>% select(test_score)
...
eff.model <- lm(test_score ~ treatment1, data=afghan)
summary(eff.model)
...
```{r}
SSE0 <- sum((gp0 - gp0.mean)^2)
SSE1 <- sum((gp1 - gp1.mean)^2)
SSError <- SSE0 + SSE1
...
*Since the SE is lower for the multiple means model, we can say that model is better than the single mean model. *
...
`r colorize("Note: these output (SST, SSM, SSE and their df) are given in what is known as an ANOVA (analysis of variance) table. More on this will come later in the course.", "red")`
...
`r colorize("Note: We should NOT use the Rsquared value from the model without the intercept (separate means models).", "red")`
<br> 
