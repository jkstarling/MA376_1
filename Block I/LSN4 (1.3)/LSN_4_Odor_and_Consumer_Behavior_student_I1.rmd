...
```{r}
df <- scent.dat %>% group_by(condition) %>% summarise(happy=mean(rating))
mydiff <- diff(df$happy)
mydiff
```
\vspace{.5in}
...
```{r}
set.seed(476)
scent.dat2 <- scent.dat %>% mutate(new_cond = sample(scent.dat$condition)) %>% select(-condition)
```

**7) Now under this labeling scheme we would have observed a difference of:**

```{r}
diffs=scent.dat2 %>% group_by(new_cond) %>% summarise(happy=mean(rating))
diff(diffs$happy)
```
...
for(i in 1:M){
  ...
  diffs = scent.dat3 %>% group_by(new_cond) %>% 
    summarise(happy=mean(rating))
  stat[i] <- diff(diffs$happy)
}
...
```{r}
p + geom_vline(xintercept = mydiff, linetype="dashed", color="red") +
  geom_vline(xintercept = -mydiff, linetype="dashed", color="red")
```
...
for(i in 1:M){
  ...
  diff.means= 2*sim.lm$coefficients[2]
