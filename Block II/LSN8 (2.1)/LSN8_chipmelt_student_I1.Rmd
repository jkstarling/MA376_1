...
```{r}
chips_long <- chipmelt %>% 
  pivot_longer(cols = -student, names_to = "chip", values_to = "time")
```
...
```{r}
chips_long %>% group_by(chip) %>% summarise(mn = mean(time))
...
eff.model <- chips_long %>% lm(time~chip, data = .)
summary(eff.model)
```
...
eff.model2 <- chips_long %>% lm(time~chip+student, data = .)
summary(eff.model2)
anova(eff.model2)
```
