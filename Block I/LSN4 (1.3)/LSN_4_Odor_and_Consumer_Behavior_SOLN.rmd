...
*a numerical characteristic of the population and sample, respectively.*
\vspace{1.in}
...
*$H_0$: there is no underlying association between scent condition and favorabilty ratings*

*$H_a$: there is an underlying association between scent condition and favorabilty ratings*
\vspace{.5in}

**4) Our *parameter* and *statistic* that addresses our hypothesis are:**

*$\mu_{no\, scent} - \mu_{scent}$*

*$\bar{y}_{no\, scent} - \bar{y}_{scent}$*
\vspace{.5in}

**5) In Section 1.1, p. 38, the authors found that the difference between the two group means is 1.3. Verify this with the data. **

```{r}
df <- scent.dat %>% group_by(condition) %>% summarise(mean=mean(rating))
mydiff <- diff(df$mean)
mydiff
```
\vspace{.5in}
...
```{r}
diffs=scent.dat2 %>% group_by(new_cond) %>% 
  summarize(mn = mean(rating))%>%
  select(mn)
diff(diffs$mn)
```
...
set.seed(376)
M <- 1000
# emp.est<-data.frame(stat=rep(NA,M))#Blank data set to fill in
stat <- rep(NA, M)
for(i in 1:M){
  scent.dat3 = scent.dat %>% mutate(new_cond=sample(scent.dat$condition))%>%
  select(-condition)
  diffs=scent.dat3 %>% 
    group_by(new_cond) %>% 
    summarize(mn = mean(rating)) %>% 
  ...
  stat[i] <- diff(diffs$mn)
}
p <- ggplot(data=as.data.frame(stat),aes(x=stat))+geom_histogram(binwidth = 0.1)
show(p)
```
\vspace{0.3in}
...
```{r}
p + geom_vline(xintercept = mydiff, linetype="dashed", 
                color = "red", size=1.0) +
  geom_vline(xintercept = -mydiff, linetype="dashed", 
                color = "red", size=1.0)
```

```{r}
(sum(stat >= mydiff) + sum(stat <= - mydiff)) / M
```

*This event looks to be very rare. It happened 0 times after simulating 1000 times.*
...
for(i in 1:M){
  scent.dat3 <- scent.dat %>% 
    mutate(new_cond=sample(scent.dat$condition))%>%
    select(-condition)
  sim.lm <- lm(rating~new_cond,data=scent.dat3)
  ...
  stat[i] <- diff.means/(est.se*sqrt(2/24))
}
```
...
```{r}
my.tstat <- mydiff / (summary(scent.mod)$sigma * sqrt(2/24))
my.tstat
(sum(stat >= my.tstat) + sum(stat <= - my.tstat)) / M
```

*This does appear to be very rare. Only 0.1% of the observation were outside of this value.*
\vspace{0.5in}
...
tstar <- qt(1-alpha/2,n1+n2-2) 
stat.se <- SSE/(n1+n2-2)*sqrt(1/n1+1/n2)
upperCI <- mydiff + stat.se*tstar
lowerCI <- mydiff - stat.se*tstar
c(lowerCI, upperCI)
```
\vspace{0.5in}
